...
%This code is the reference of the paper:"Two-phase robust target localization in ocean sensor networks using
...
L_noise=length(noi);
%% RSSD_TDOA_IPM
mse_RSSD_TDOA_IPM_x = zeros(L_noise,1);
mse_RSSD_TDOA_IPM_t = zeros(L_noise,1);
IPM_CDF=zeros(N_MC,1);
warning off
for nn2=1:L_noise
    ...
    for mc=1:N_MC % loop for Monte Carlo trials
        ...
        Objective= @(xx)barrierf2(mixA2,mixb2,xx);
        [Result,fval,exitflag] = fmincon(Objective,x0,[],[],[],[],[],[],[],options);
        x_t = Result(1:2);
        mse_RSSD_TDOA_IPM_t(nn2) = mse_RSSD_TDOA_IPM_t(nn2)+toc;
        % Calculate mse of x
        mse_RSSD_TDOA_IPM_x(nn2)=mse_RSSD_TDOA_IPM_x(nn2)+sum((x_t-x).^2);%
        IPM_CDF(mc)=sqrt(sum(x_t-x).^2);
    end
end