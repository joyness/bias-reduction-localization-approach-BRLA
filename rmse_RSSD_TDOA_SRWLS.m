...
L_noise=length(noi);
%% RSSD_TDOA_SRWLS
mse_RSSD_TDOA_SRWLS_x = zeros(L_noise,1);
mse_RSSD_TDOA_SRWLS_t = zeros(L_noise,1);
SRWLS_CDF=zeros(N_MC,1);
warning off
for nn2=1:L_noise
    ...
    for mc=1:N_MC % loop for Monte Carlo trials
        ...
        %%TDOA
        RD=r-ones(m,1).*r1+noise1;%the noise TDOA measurement with respect to the first sensor
        ...
        %% SRWLS
        tic
        [y_tilde] = MPUpositionSRWLS(mixA2,mixb2,W);
        x_t  = y_tilde;
        mse_RSSD_TDOA_SRWLS_t(nn2) = mse_RSSD_TDOA_SRWLS_t(nn2)+toc;
        % Calculate mse of x
        mse_RSSD_TDOA_SRWLS_x(nn2)=mse_RSSD_TDOA_SRWLS_x(nn2)+sum((x_t-x).^2);%
        SRWLS_CDF(mc)=sqrt(sum(x_t-x).^2);
    end
end
